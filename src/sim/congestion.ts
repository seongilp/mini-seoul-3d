...
export class Congestion {
   ...
   * 자료가 없으면 null 을 돌려주므로 호출하는 쪽에서 원래 색을 쓰면 된다.
   */
  ratioAt(routeId: string, dir: 1 | -1, hour: number, along: number): number | null {
...
export function congestionLabel(ratio: number): string {
  ...
  if (ratio < 0.9) return "보통";
  if (ratio < 1.3) return "혼잡";
  if (ratio < 1.8) return "매우 혼잡";
  return "극심";
}
